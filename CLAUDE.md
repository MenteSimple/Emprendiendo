...
```bash
grep -rniE "uniandes|universidad|facultad|\badmi\b|\bnrc\b|bloque ne[óo]n|brightspace|syllabus|la entrega|la sesi[óo]n|estudiante|alumn[oa]|profesor|docente|\bcursos?\b|semestre|r[úu]brica|calificaci[óo]n|matriculad|c[áa]tedra" --include='*.md' --include='*.txt' --include='*.py' .
```

Los `.py` van incluidos porque los generadores de plantilla llevan el texto adentro, y de ahí pasa a las celdas: si el patrón no los mira, no puede ver lo que termina publicado en un `.xlsx`.

Varios de esos términos dan positivos legítimos —«estudiantes que viven solos» es un público objetivo como cualquier otro, y hoy aparece en tres archivos—. Lo que hay que cazar es que así se le diga a **quien lee**: eso presupone matrícula, y estuvo seis veces en el prompt de la 01 sin que nadie lo notara. El `grep` avisa; quien decide es una persona. Un patrón que grita en falso tres veces enseña a ignorarlo la cuarta.
