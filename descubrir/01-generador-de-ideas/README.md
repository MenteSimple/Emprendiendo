...
PASO 4B — EL ARCHIVO. Tres rutas, en este orden de preferencia. Toma la primera que puedas.
  ...
  RUTA 1 — LLENAR LA PLANTILLA ADJUNTA. Tiene tres hojas —`Molestias`, `Ideas`, `SCAMPER`— con los encabezados ya puestos.
     ...
     Tiene que sobrevivir todo esto:
        ...
        · la celda `Nombre completo` de la hoja `Ideas`, que la lleno yo y va vacía.
     Escribe desde la fila 2, así:
        ...
        `★` — vacía siempre. Esa la marco yo.

     ⛔ ORIGEN = X EN LAS 34, SIN EXCEPCIÓN. La plantilla explica que `P` es para lo que observé yo y `S` para lo que yo saqué de SCAMPER. Esas dos letras son para MI trabajo, no para el tuyo: todo lo que generaste tú es `X`, incluidas las de SCAMPER, porque las produjiste tú y no yo. Marcarlas `P` o `S` me atribuye ideas que no son mías y arruina lo único que ese registro sirve para saber: cuáles ideas sobrevivieron al filtro, si las mías o las de la máquina. Ante la duda, `X`.
     ...
     ⛔ Si no pudiste abrir el archivo, escribe exactamente `NO ABRÍ EL ARCHIVO` y no inventes las celdas. Inventarlas es mucho peor que reconocerlo: me quedo con un archivo roto creyendo que está verificado.
  ...
  RUTA 3 — SOLO TEXTO. Si no puedes generar ningún archivo, dilo en una sola línea: "no puedo generar archivos". Los bloques bastan y los pego a mano.
